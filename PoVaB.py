...
for i,kpoint in enumerate(projections['spagetti'][0]):
    data.append([])
    for b,band in enumerate(kpoint):
        data[i].append(None)
        ...
        if occ < 1e-4:
            continue
        ax.scatter(TOTAL_KPOINTS*np.sum(mul[:i//TOTAL_KPOINTS])+(i%TOTAL_KPOINTS)*mul[i//TOTAL_KPOINTS], eigenvalues['spagetti'][0][i][b][0]-efermi,s=10*np.power(2*occ,2), c='k', alpha=0.9)
